...
8. 列表页、弹窗列表、左侧筛选区等所有搜索框默认提供清除按钮，`AppInput` 使用 `:input-props="{ clearable: true }"`，搜索型 `AppSelect` 使用 `:select-props="{ clearable: true }"`。
9. 输入类组件的 placeholder 统一写成“请输入 + 表单名称”，例如“请输入用户名”“请输入角色编码”。
10. 选择类组件的 placeholder 统一写成“请选择 + 表单名称”，例如“请选择所属组织”“请选择菜单类型”。
11. 如果字段名称较长，placeholder 也应保留完整业务语义，不要随意缩写成模糊表述。
12. 查询分页、查询详情、列表刷新这类请求，发起前必须显式开启 loading，并在 `finally` 中关闭 loading。
13. 页面列表型请求默认维护页面级 loading，例如表格 `:loading=\"loading\"`，不要只请求不显示加载态。
14. 点击按钮触发的异步事件，如果会发请求，必须增加对应的按钮 loading 或操作 loading，并绑定到按钮上。
15. 所有引入 loading 的异步请求，统一包裹 `try/finally`，只在 `finally` 中关闭 loading，避免中途 `return` 导致 loading 残留。
16. 新增、编辑、删除、重置、刷新成功后，如果外层有列表、树或汇总数据，必须同步刷新外层数据，不要只关弹窗不刷新页面。
17. 表单弹窗的提交 loading 必须覆盖到父级真实请求完成，子组件触发 `emit('submit')` 时需要 `await` 父组件返回的 Promise。
